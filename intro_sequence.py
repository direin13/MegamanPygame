...
class Stage_rectangle(Megaman_object):
   def __init__(self, y, stage_name='none'):
      super().__init__('stage_rectangle', x=0, y=y, width=600, height=0, display_layer=2)
      self.stage_name = Bit_text(stage_name, 140, self.y + self.height//2 + 8*12-4, 3, 3, pattern_interval=10)
      self.all_timers = Timer()
      self.all_timers.add_ID('time_till_start_sequence', 50)
      self.all_timers.add_ID('show_stage_name', 30)
      self.y_offset = self.y - self.height//2

   def display(self, surf):
      if self.all_timers.is_empty('time_till_start_sequence'):
         pygame.draw.rect(surf, (0, 53, 200), (self.x, self.y_offset, self.width, self.height))
      pygame.draw.rect(surf, (0, 63, 210), (self.x, self.y_offset - 8, self.width, 4))
      ...
      pygame.draw.rect(surf, (0, 53, 200), (self.x, self.y + self.height//2 + 4, self.width, 4))
