...
class Megaman_object(Sprite_surface):
   ...
   def apply_gravity(self):
      ...
      if -(self.y_vel) >= Megaman_object.gravity_speed:
         ...
         if self.all_timers.is_finished('y_accel_flag') != True:
            self.y += -(self.y_vel)
   ...
   def accelerate(self, acc_speed=0, max_x_vel=0):
      ...
      if acc_speed != 0:
         ...
         if self.all_timers.is_finished('move_flag'):
   ...
   def hone_in(self, x=None, y=None, decrease_scale=10, min_x_vel=1, min_y_vel=1, max_x_vel=1, max_y_vel=1):
      x_vel = abs(self.x - x)//decrease_scale
      y_vel = abs(self.y - y)//decrease_scale
