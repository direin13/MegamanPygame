...
class Bit_text(object):
   ...
   def flash(self, surf, pattern_interval=None):
      if pattern_interval != None:
         self.pattern_timers.all_timers['flash_interval'] = pattern_interval

      if self.pattern_timers.is_almost_finished('flash_interval', self.pattern_timers.all_timers['flash_interval']//2) != True:
         Bit_text.display_text(surf, (self.x,self.y), self.string, self.width, self.height, self.colour)

      elif self.pattern_timers.is_empty('flash_interval'):
   ...
   def show_letter_sequence(self, surf, pattern_interval=None):
      if pattern_interval != None:
         self.pattern_timers.all_timers['add_letter_interval'] = pattern_interval
      s = self.string[0:self.show_letters_up_to]
      if self.show_letters_up_to == len(self.string):
         s = self.string
      else:
         if self.pattern_timers.is_empty('add_letter_interval') != True:
