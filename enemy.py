...
class Enemy(Character):
   ...
   def display_explosions(self, surf):
      ...
      for timer in self.explosion_timers:
         if self.explosion_timers.is_empty(timer) != True:
...
class Met(Enemy):
   sprite_img = load_images('enemies/met')
...
class Detarnayappa(Enemy):
   sprite_img = load_images('enemies/detarnayappa')
   ...
   def display(self, surf):
      if self.all_timers.is_full('stop_time') or self.all_timers.is_empty('stop_time') and universal_var.game_pause != True:
...
class Hoohoo(Enemy):
   sprite_img = load_images('enemies/hoohoo')
   ...
   def display(self, surf):
      ...
      if self.is_alive():
         ...
         if self.boulder_drop and self.all_timers.is_empty('drop_boulder_animation') != True:
   ...
   def update(self):
      #self.all_timers.print_timer('start_time')
      ...
      if self.all_timers.is_empty('start_time') != True:
      ...
      if init and universal_var.game_pause != True and self.is_alive():
         if self.all_timers.is_empty('fly_down') != True:
            self.move(x_vel=self.max_x_vel, y_vel=2)
            self.all_timers.countdown('fly_down')
         elif self.all_timers.is_empty('fly_up') != True:
            self.move(x_vel=self.max_x_vel, y_vel=-2)
...
class Paozo(Enemy):
   sprite_img = load_images('enemies/paozo')
   ...
   def display(self, surf):
      ...
      if self.direction == True:
         ...
         flip = False
      if self.all_timers.is_empty('no_display') == True and self.is_alive():
         ...
         elif self.all_timers.is_empty('phase_1_wait') != True or self.all_timers.is_empty('phase_2_flip_animation'):
   ...
   def respawn(self):
      if universal_var.game_pause != True and universal_var.game_reset != True and self.is_on_screen(universal_var.screen_width, universal_var.screen_height):
         ...
         elif self.all_timers.is_empty('respawn_obj_collision') != True:
   ...
   def update(self):
      if self.is_alive() != True:
         ...
         if self.all_timers.is_empty('phase_1_wait') != True:
            self.all_timers.countdown('phase_1_wait')
         elif self.all_timers.is_full('phase_3_begin'):
            self.ball.set(self.ball.x, self.ball.y, 40, 0)
            self.ball.flipped = True
            self.all_timers.countdown('phase_3_begin')
         else:
            if self.all_timers.is_empty('phase_3_begin') != True:
               self.all_timers.countdown('phase_3_begin')
            elif self.vacuum_mode != True:
               self.ball.set(self.ball.x, self.ball.y, 40, 180)
         ...
         if self.ball.x <= self.ball.spawn_point[0] and self.ball.flipped and self.vacuum_mode:
            self.ball.launched = False
            self.ball.flipped = False
            self.ball.x = self.ball.spawn_point[0]
            self.vacuum_mode = False
...
class Paozo_Ball(projectile.Projectile, Enemy):
   def __init__(self, x, y):
      ...
      super().__init__('ball', x, y, [main_sprite], [main_collbox], is_active, width, height, display_layer)
...
class Big_stomper(Enemy):
   sprite_img = load_images('enemies/big_Stomper')
   ...
   def display(self, surf):
      if self.all_timers.is_empty('no_display') and self.is_alive():
         if universal_var.game_pause != True:
            self.update_sprite(universal_var.main_sprite)
         ...
         elif self.all_timers.is_empty('prepare_to_jmp') or self.all_timers.is_empty('wait') != True:
   ...
   def check_all_collisions(self):
      ...
      if self.grounded == False and self.is_alive():
         if self.all_timers.is_empty('y_accel_flag'):
            self.all_timers.replenish_timer('y_accel_flag')
            self.y_vel -= 2
         else:
      ...
      if self.is_alive() and universal_var.game_reset != True and universal_var.game_pause != True:
         if self.grounded:
            ...
            if self.all_timers.is_empty('prepare_to_jmp') != True:
               ...
               if self.all_timers.is_empty('wait') != True:
