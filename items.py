      ...
      if universal_var.game_reset != True:
         ...
         if self.is_drop:
            if (self.is_active and (self.is_on_screen(universal_var.screen_width, universal_var.screen_height) and 
               self.all_timers.is_empty('time_till_vanish') == False)):
...
class Health_capsule(Item):
   ...
   def display(self, surf):
      ...
      if self.is_active and universal_var.game_reset == False and camera_transitioning() == False and self.count == 0:
   ...
   def update(self):
      if self.is_active:
         ...
         if megaman_collision.is_empty() != True:
            ...
            if self.count != self.threshold and m.health_points != m.health_points_copy and self.all_timers.is_full('increment'):
               self.count += 1
               m.health_points += 1
               m.health_bar.points += 1
            ...
            elif self.count == self.threshold or m.health_points == m.health_points_copy:
               self.is_active = False
               self.count = 0
               universal_var.game_pause = False

            if self.all_timers.is_empty('increment'):
